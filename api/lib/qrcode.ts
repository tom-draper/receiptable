...
export default function qrCodeSvg(r: string | QRCodeOptions) {
    let n: number,
        t: any,
        o: number,
        e: number,
        ...
        w = function (r: number): void {
            for (var n = [function (): number { return 0 == (t + o) % 2 ? 1 : 0 }, function (): number { return 0 == t % 2 ? 1 : 0 }, function (): number { return 0 == o % 3 ? 1 : 0 }, function (): number { return 0 == (t + o) % 3 ? 1 : 0 }, function (): number { return 0 == ((t / 2 | 0) + (o / 3 | 0)) % 2 ? 1 : 0 }, function (): number { return 0 == t * o % 2 + t * o % 3 ? 1 : 0 }, function (): number { return 0 == (t * o % 2 + t * o % 3) % 2 ? 1 : 0 }, function (): number { return 0 == ((t + o) % 2 + t * o % 3) % 2 ? 1 : 0 }][r], t = e; t--;)for (var o = e; o--;)f[t][o] || (a[t][o] ^= n());
        },
        b = function (): number {
            for (var r = function (r: number, n: number[]): void {
                n[6] || (r += e), n.shift(), n.push(r);
            },
                n = function (n: boolean, o: number, a: number[]): number {
                    return n && (r(o, a), o = 0), r(o += e, a), t(a);
                },
                t = function (r: number[]): number {
        ...
        Q = function (r: number[]): number[] {
            ...
            for (var o = d[t[0]][n], e = g[t[0]][n], a = D(n) / 8 | 0, f = o - a % o, i = a / o | 0, u = [], h = function (r: number): number[] {
                var n = 1, t: number[] = [];
                t[r - 1] = 1;
                for (var o = 0; o < r; o++) {
                    for (var e = 0; e < r; e++)t[e] = p(t[e], n) ^ (t[e + 1] || 0);
                    n = p(n, 2);
                }
                return t;
            }(e),
        ...
        V = function (r: number, n: number[]): number {
            return (D(r) / 8 | 0) - g[n[0]][r] * d[n[0]][r];
        },
        E = function (r: number, n: number, t: number | boolean): void {
            a[n][r] = t ? 1 : 0, f[n][r] = 1;
        },
        ...
        z = function (r: string): any {
            ...
            return n < r.length && A(l.indexOf(r.charAt(n)), 6, t), { modeBits: 2, numBitsCharCount: [9, 11, 13], numChars: r.length, bitData: t };
        },
        L = function (r: string, n: number[], t: number, o: number): void {
            var e = function (r: string): any[] {
                return "" == r ? [] : c.test(r) ? [Z(r)] : s.test(r) ? [z(r)] : [R(S(r))];
            }(r);
            return U(e, n, t, o);
        },
        N = function (r: number, i: number[], u: number[], h: number): void {
            t = i, o = h;
            for (var v = e = 4 * (n = r) + 17; v--;)a[v] = [], f[v] = [];
            if (O(), function (r: number[]): void {
        ...
        U = function (r: any[], n: number[], t: boolean, o: number, e?: number, a?: number): void {
    ...
    return function () {
        ...
        let pathData = "";
        const options = typeof r === "string" ? { msg: r } : r || {};
        const colors = options.pal || ["#484848"];
        const size = h(options.dim) || 40;
        const padding = (h(options.pad) > -1) ? h(options.pad) : 0;
        const transform = [1, 0, 0, 1, padding, padding];
        const fill = colors[0] ? colors[0] : "#484848";
        const background = colors[1] ? colors[1] : null;
        const compact = options.vrb ? 0 : 1;

        // Generate QR code data
        L(options.msg || "", m[options.ecl as keyof typeof m] || m.M, options.ecb === 0 ? 0 : 1, options.mtx as number);

        const totalSize = e + 2 * padding;
            ...
            for (let u = 0, f = e; f--;) {
                if (a[i][f]) {
                    if (compact) {
                        u++;
                        if (!a[i][f - 1]) {
                            pathData += "M" + f + "," + i + "h" + u + "v1h-" + u + "v-1z";
                            u = 0;
                        }
                    } else {
                        pathData += "M" + f + "," + i + "h1v1h-1v-1z";
                    }
                }
            }
        }
        ...
        let svgString = `<svg xmlns="${s}" viewBox="${viewBox}" width="${size}%" fill="${fill}" shape-rendering="crispEdges" version="1.1">`;

        if (background) {
            svgString += `<path fill="${background}" d="M0,0V${totalSize}H${totalSize}V0H0Z"/>`;
        }

        svgString += `<path transform="matrix(${transform})" d="${pathData}"/>`;
        svgString += '</svg>';

        // Return object with SVG info
        return svgString;
    }();
}